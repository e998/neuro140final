...
class Agent:
    ...
    def getAction(self, state):
        # Agent receives a GameState and must return action from
...
class Configuration:
    # Configuration captures (x,y) coordinate of agent and traveling direction.
    ...
    def generateSuccessor(self, vector):
        # Next, "successor" configuration defined by translating current with action vector.
        # Not a check for legality of movement.
        # Action is defined as a movement vector.
        x, y = self.pos
        dx, dy = vector
...
class AgentState:
    # State of agent: configuration, speed, isScared
...
class Grid:
    # 2D array of objects with list of lists
    # grid[x][y] where (x,y) is position on map
    # __str__ method constructs output oriented like pac-man board
    ...
    def packBits(self):
        # Efficient int list representation: (width, height, bitPackedInts,...)
    ...
    def _unpackBits(self, bits):
        # Data unpacked from bit-level representation
        cell = 0
        for packed in bits:
...
### Below should not need further adjustment
