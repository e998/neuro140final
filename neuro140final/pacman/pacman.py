...
def readCommand(argv):
    ...
    parser.add_option('-s', '--ghostsTest', dest='ghostTest',
                    help=default(
                        'the ghost agent TYPE in the ghostAgents module to use for testing'),
                    metavar='TYPE', default='RandomGhost')
    ...
    # Choose a ghost agent for training
    ghostType = loadAgent(options.ghost, noKeyboard)
    args['ghosts'] = [ghostType(i + 1) for i in range(options.numGhosts)]

    # Load ghost agents for testing
    ghostTestType = loadAgent(options.ghostTest, noKeyboard)
    args['ghostsTest'] = [ghostTestType(i + 1) for i in range(options.numGhosts)]
...
def runGames(layout, pacman, ghosts, ghostsTest, display, numGames, record, numTraining=0, catchExceptions=False, timeout=30):
    import __main__
    ...
    for i in range(numGames):
        ...
        # Select ghosts for training
        if beQuiet:
            game = rules.newGame(layout, pacman, ghosts,
                             gameDisplay, beQuiet, catchExceptions)
        # Select ghosts for testing
        else:
            game = rules.newGame(layout, pacman, ghostsTest,
                             gameDisplay, beQuiet, catchExceptions)
